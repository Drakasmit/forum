	...
	<div class="sv-forum-block">
		<div class="sv-forum_header sv-forum_header-subject_page">
			<div class="sv-left_bg">
				<h2>{$aLang.forum_new_topic_for} <a href="{router page='forum'}{$oForum->getId()}/">{$oForum->getTitle()}</a></h2>
			</div>
				...
				<form action="" method="POST" enctype="multipart/form-data">
					<fieldset>
						...
						<p>
							...
							<span class="form_note">{$aLang.forum_new_topic_title_notice}</span>
						</p>

						{if !$oConfig->GetValue('view.tinymce')}<div class="note">{$aLang.topic_create_text_notice}</div>{/if}
						<label for="topic_text">{$aLang.forum_post}:</label>
